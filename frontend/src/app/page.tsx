  ...
  return (
    <div className="min-h-screen pb-14">
      ...
      <main className="mx-auto w-full max-w-6xl px-5 pt-10 md:px-8 md:pt-16">
        <section className="hero-glow glass-header reveal-up overflow-hidden p-7 md:p-11">
          <div className="grid items-center gap-10 md:grid-cols-[1.12fr_0.88fr]">
            <div>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-white/10 px-3 py-1.5 text-xs font-semibold text-cyan-100">
            ...
            <div className="card float-soft rounded-3xl p-5 shadow-[0_18px_42px_rgba(15,118,110,0.14)]">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-300">Live Pulse Preview</p>
                ...
                {[
                  ...
                  <div key={label} className="panel rounded-2xl p-4">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-2xl font-semibold text-white [font-family:var(--font-heading)]">{score}</span>
                    </div>
        ...
        <section className="mt-8 grid gap-3 md:grid-cols-3">
          {metrics.map((item, idx) => (
            <div key={item.label} className="card reveal-up p-5" style={{ animationDelay: `${idx * 0.08}s` }}>
              ...
              <p className="mt-3 text-2xl font-semibold text-white [font-family:var(--font-heading)]">{item.value}</p>
            </div>
          ))}
        </section>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          {features.map((item, idx) => (
            <article key={item.title} className="card reveal-up p-6" style={{ animationDelay: `${0.15 + idx * 0.07}s` }}>
              ...
              <h2 className="mt-4 text-lg font-semibold text-white [font-family:var(--font-heading)]">{item.title}</h2>
