  ...
  return (
    <Link
      ...
      className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-all ${
        active
          ? 'bg-white/14 text-white shadow-[0_16px_24px_rgba(6,12,24,0.32)] ring-1 ring-white/15'
  ...
  return (
    <div className="min-h-screen px-4 py-4 md:px-6 md:py-6">
      <div className="mx-auto grid w-full max-w-[1320px] gap-4 md:grid-cols-[250px_1fr]">
        <aside className="glass-header flex flex-col p-3 md:p-4">
          <div className="panel mb-4 p-3">
            <FlowPulseLogo />
            <p className="mt-2 truncate text-xs text-slate-400">{user?.organization_name || 'Workspace'}</p>
          </div>
          ...
          <div className="mt-4 border-t border-white/10 pt-4">
            <div className="panel flex items-center gap-3 p-2.5">
              {user?.avatar_url ? (
                ...
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-cyan-200/15 text-sm font-semibold text-cyan-100">
                  {user?.name?.[0] || 'U'}
                </div>
              )}
        ...
        <main className="glass-header min-h-[80vh] overflow-hidden p-4 md:p-7">{children}</main>
      </div>
    </div>
  );
}
