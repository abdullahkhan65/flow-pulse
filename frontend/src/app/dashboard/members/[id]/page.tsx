...
function StatCard({ label, value, sub, icon }: { label: string; value: string | number; sub?: string; icon: React.ReactNode }) {
  return (
    <div className="card p-5">
      ...
      <p className="mt-2 text-3xl font-semibold [font-family:var(--font-heading)]">{value}</p>
    ...
    <div className="space-y-5 reveal-up">
      <div className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white/70 p-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">Historical weekly score trend and recent activity breakdown.</p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-medium text-slate-600">Meetings Attended</p>
          </div>
          <p className="text-2xl font-semibold [font-family:var(--font-heading)]">
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-medium text-slate-600">Emails</p>
          </div>
          <p className="text-2xl font-semibold [font-family:var(--font-heading)]">
        ...
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-medium text-slate-600">Tasks Completed</p>
          </div>
          <p className="text-2xl font-semibold [font-family:var(--font-heading)]">
      ...
      <section className="card overflow-hidden">
        <div className="border-b border-slate-200 px-5 py-4">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Burnout risk and load scores per completed week</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-2.5">Week</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Risk</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Meetings</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Focus</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">After-hrs</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Delta</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {(scores?.weeklyScores || []).map((w) => (
                <tr key={w.week_start} className="hover:bg-slate-50/80 transition-colors">
                  <td className="px-5 py-3 font-medium text-slate-800">{format(parseISO(w.week_start), 'MMM d, yyyy')}</td>
                  <td className="px-3 py-3 text-slate-600">{Math.round(w.burnout_risk_score)}</td>
                  <td className="px-3 py-3 text-slate-600">{Math.round(w.meeting_load_score)}</td>
                  <td className="px-3 py-3 text-slate-600">{Math.round(w.focus_score)}</td>
                  <td className="px-3 py-3 text-slate-600">{Math.round(w.after_hours_score)}</td>
      ...
      <section className="card overflow-hidden">
        <div className="border-b border-slate-200 px-5 py-4">
          <h2 className="text-lg font-semibold [font-family:var(--font-heading)]">Weekly Activity Breakdown</h2>
          <p className="text-xs text-slate-500 mt-0.5">Raw counts of meetings, emails, tasks, and GitHub activity per week</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                ...
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Mtg hrs</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">
                  <div className="flex items-center gap-1"><Mail className="h-3 w-3 text-sky-500" /> Sent</div>
                </th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Received</th>
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">
                ...
                <th className="text-left text-xs font-medium text-slate-500 px-3 py-2.5">Reviews</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {(scores?.weeklyActivity || []).map((a) => {
                ...
                return (
                  <tr key={weekKey} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-5 py-3 font-medium text-slate-800">{format(parseISO(weekKey), 'MMM d, yyyy')}</td>
                    <td className="px-3 py-3 text-slate-700">{a.meeting_count}</td>
                    <td className="px-3 py-3 text-slate-500">{Math.round(a.total_meeting_minutes / 60 * 10) / 10}h</td>
                    <td className="px-3 py-3 text-slate-700">{a.emails_sent}</td>
                    <td className="px-3 py-3 text-slate-500">{a.emails_received}</td>
                    <td className="px-3 py-3 text-slate-700">{a.tasks_completed}</td>
                    <td className="px-3 py-3 text-slate-700">{a.commits}</td>
                    <td className="px-3 py-3 text-slate-700">{a.prs_created}</td>
                    <td className="px-3 py-3 text-slate-500">{a.pr_reviews}</td>
                  </tr>
                );
              })}
