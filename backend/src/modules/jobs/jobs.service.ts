    ...
    try {
      ...
      for (const user of users) {
        await this.syncUser(user.user_id, user.organization_id).catch((err) => {
          this.logger.error(
            `Failed to sync user ${user.user_id}: ${err.message}`,
          );
        });
  ...
  async runDailyAggregation() {
    ...
    for (const user of result.rows) {
      await this.analyticsService
        ...
        .catch((err) => {
          this.logger.error(
            `Daily aggregation failed for user ${user.user_id}: ${err.message}`,
          );
        });
    }

    this.logger.log(
      `Daily aggregation complete for ${result.rows.length} users`,
    );
  }
  ...
  async runWeeklyScores() {
    ...
    for (const user of result.rows) {
      await this.analyticsService
        ...
        .catch((err) => {
          this.logger.error(
            `Weekly score failed for user ${user.user_id}: ${err.message}`,
          );
        });
    ...
    for (const orgId of orgSet) {
      await this.analyticsService
        ...
        .catch((err) => {
          this.logger.error(
            `Team score failed for org ${orgId}: ${err.message}`,
          );
        });
    }
