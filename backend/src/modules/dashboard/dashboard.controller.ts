...
export class DashboardController {
  ...
  @ApiOperation({
    summary:
      "Trigger immediate sync + build aggregates. Returns preview data. Safe to call repeatedly.",
  })
  ...
  @ApiOperation({
    summary:
      "Get partial week scores + today snapshot. Works from day 1 after first sync.",
  })
  getPreview(@CurrentUser() user: JwtPayload) {
    return this.dashboardService.getPreview(user.sub);
  }
  ...
  @Roles("manager")
  @ApiOperation({
    summary: "Get team dashboard (4-week view + current week in progress)",
  })
  getTeamDashboard(
  ...
  @Roles("manager")
  @ApiOperation({
    summary: "Get all team members with latest scores (no ranking)",
  })
  ...
  @ApiOperation({
    summary:
      "Trigger immediate sync for all active org members. Returns counts of succeeded/failed.",
  })
  ...
  getTeamCalendar(
    ...
    return this.dashboardService.getTeamCalendar(
      user.organizationId,
      startDate,
    );
  }
  ...
  @Header(
    "Content-Disposition",
    'attachment; filename="flowpulse-team-export.csv"',
  )
  @ApiOperation({ summary: "Export team scores as CSV" })
  exportCsv(@CurrentUser() user: JwtPayload, @Query("weeks") weeks?: string) {
  ...
  @ApiOperation({
    summary:
      "Get tickets completed this week + currently in-progress from Jira. Titles fetched live, never stored.",
  })
