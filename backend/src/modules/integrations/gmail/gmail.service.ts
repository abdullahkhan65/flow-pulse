...
export class GmailService {
  ...
  async syncUserEmails(
    ...
    if (!tokens) {
      this.logger.debug(
        `No Google tokens for user ${userId} — skipping Gmail sync`,
      );
      return { synced: 0 };
    }
    ...
    try {
      ...
      for (const detail of sentDetails) {
        ...
        const recipientCount =
          toHeader.split(",").length +
          (ccHeader ? ccHeader.split(",").length : 0);

        const { weekday } = this.getHourAndWeekdayInTimezone(
          occurredAt,
          userTimezone,
        );
        events.push({
          ...
          occurredAt,
          isAfterHours: this.isAfterWorkHours(
            occurredAt,
            tokens.orgSettings,
            userTimezone,
          ),
