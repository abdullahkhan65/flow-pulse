...
export class JiraService {
  ...
  private async getTokenAndAccountId(userId: string): Promise<{ token: string; cloudId: string; accountId: string } | null> {
    ...
    if (expiresAt && expiresAt.getTime() < Date.now() + 5 * 60 * 1000) {
      if (row.refresh_token) {
        try {
          ...
          this.logger.warn(`Failed to refresh Jira token for user ${userId}: ${detail}`);
          await this.db.query(
            `UPDATE integrations SET status = 'error', error_message = $1, updated_at = NOW() WHERE user_id = $2 AND type = 'jira'`,
            ['Token refresh failed — please reconnect Jira in Settings', userId],
          );
          return null;
        }
      } else {
        this.logger.warn(`Jira token expired for user ${userId} and no refresh_token stored`);
      }
    }
  ...
  async syncUserActivity(userId: string, orgId: string) {
        ...
        if (statusChanges.length > 0) {
          ...
          logs.push({
            ...
            metadata: {
              ...
              fromStatus,
              toStatus,
              isCompleted: DONE_STATUSES.some((s) => s.toLowerCase() === toStatus.toLowerCase()),
    ...
    for (const issue of allAssignedRes.data.issues || []) {
      ...
      const statusCategory: string = issue.fields.status?.statusCategory?.key ?? 'undefined';
      ...
      const categoryName =
        ...
        statusCategory === 'done' ? 'Done' : 'Other';
    ...
    this.logger.log(`Synced ${logs.length} Jira transition events + ${snapshotLogs.length} ticket state snapshots for user ${userId}`);
  ...
  async getJiraTicketSummary(userId: string): Promise<JiraTicketSummary> {
    ...
    if (!ctx) {
      return {
        ...
        toDo: [],
        inProgress: [],
        workload: { todoCount: 0, inProgressCount: 0, completedThisWeekCount: 0, afterHoursUpdates: 0, weekendUpdates: 0 },
        velocity: { completedCount: 0, afterHoursTransitions: 0, weekendTransitions: 0, totalTransitionsThisWeek: 0 },
        connected: false,
      };
    }
    ...
    const velocity = {
      ...
      afterHoursTransitions: allTransitions.filter((r) => r.is_after_hours).length,
    ...
    const afterHoursUpdates = snapshotRows.filter((r) => r.metadata?.lastUpdatedAfterHours === true).length;
    const weekendUpdates = snapshotRows.filter((r) => r.metadata?.lastUpdatedWeekend === true).length;
    const todoCount = snapshotRows.filter((r) => r.metadata?.statusCategory === 'To Do').length;
    const inProgressCount = snapshotRows.filter((r) => r.metadata?.statusCategory === 'In Progress').length;
    const doneTodayCount = snapshotRows.filter((r) => r.metadata?.statusCategory === 'Done').length;

    const workload = {
      todoCount,
      inProgressCount,
      completedThisWeekCount: Math.max(completedThisWeek.length, doneTodayCount),
      afterHoursUpdates,
      weekendUpdates,
    };
    ...
    try {
      const [toDoRes, inProgressRes] = await Promise.all([
        axios.get(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/search/jql`, {
          headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
          params: {
      ...
      const detail = err.response?.data ? JSON.stringify(err.response.data) : err.message;
      this.logger.warn(`Failed to fetch live Jira tickets for user ${userId}: ${detail}`);
    }

    return { completedThisWeek, toDo, inProgress, workload, velocity, connected: true };
  }
}
